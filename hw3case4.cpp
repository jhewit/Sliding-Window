...
int main( int argc, char *argv[] ) {
  ...
  cerr << "   4: sliding windows, 1 & 30 only" << endl;
  cerr << "--> ";
  cin >> testNumber;
  ...
  if ( myPart == CLIENT ) {
    ...
    case 4:
      timer.start( );
      retransmits =
      clientSlidingWindow( sock, MAX, message, 1 );
      cerr << "Drop percentage = ";                              // lap timer
	  cout << (((double)retransmits/MAX)*100) << " ";
	  cerr << "Elasped time = "; 
	  cout << timer.lap( ) << endl;

      timer.start( );
      retransmits =
      clientSlidingWindow( sock, MAX, message, 30 );
       cerr << "Drop percentage = ";                              // lap timer
	  cout << (((double)retransmits/MAX)*100) << " ";
	  cerr << "Elasped time = "; 
	  cout << timer.lap( ) << endl;
