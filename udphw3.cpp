...
{
    cerr << "Beginning the transmissions..." << endl;
    for (int i = 0; i < max; i++)
    {
        if (message[0] == (max - 1))
        {
        ...
        if (message[0] == i)
        {
            cerr << "Send ACK for message: " << message[0] << endl;
...
{
    cerr << "Starting Sliding Window..." << endl;
    int count = 0;
    int i = 0;
    int unAck = 0;
    Timer timer;
        ...
        {
            {
                ...
                while (sock.pollRecvFrom() < 1)
                {
                    ...
                    if (timer.lap() >= 1500)
                    {
                        cerr << "Timeout occurred. Retransmitting..." << endl;
                        count ++;
                        break;
                    }
                }
